...
function renderSplit() {
  ...
  async function loadFile(path) {
    ...
    document.getElementById('split-file').innerHTML = `
      <div class="file-item">
        ...
        <span style="font-size:0.75rem;color:var(--warning)" id="page-count">⏳ Contando páginas...</span>
    ...
    if (!info.ok || info.pages === 0) {
      countEl.textContent = '✗ Error al leer el PDF';
      countEl.style.color = 'var(--error)';
      notify.error('No se pudo leer el PDF. Verifica que no esté protegido.');
      return;
    }

    totalPages    = info.pages;
    customRanges  = `1-${totalPages}`;
    countEl.textContent = `✓ ${totalPages} páginas`;
    countEl.style.color = 'var(--success)';
    notify.info(`PDF cargado: ${totalPages} páginas detectadas.`);

    document.getElementById('split-wizard').style.cssText = 'display:flex;flex-direction:column;gap:1.25rem';
  ...
  function updateSummary() {
    ...
    el.innerHTML = html;
    document.getElementById('run-split').disabled = !(file && state.outputDir && valid && !state.isProcessing);
  }
    ...
    if (!res.ok) {
      ...
      document.getElementById('split-result').innerHTML =
        (ok.length  ? resultBox('success',
          `✓ ${ok.length} archivo(s) generado(s)${res.wasCancelled ? ' (cancelado antes de terminar)' : ''}`,
          ok.map(r => `📄 ${basename(r.out)}`), true) : '') +
        (err.length ? resultBox('error', `✗ ${err.length} error(es)`, err.map(r => String(r.error))) : '');

      if (ok.length)  notify.success(`${ok.length} archivo(s) separado(s) correctamente.`);
      if (err.length) notify.error(`${err.length} página(s) fallaron.`);
    }
